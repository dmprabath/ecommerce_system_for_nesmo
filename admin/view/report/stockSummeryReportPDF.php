...
$dbobj = DB::connect();
if($category==""){
    $sql = "SELECT prod_id,prod_modal,cat_name,prod_qty FROM tbl_products pro JOIN tbl_category cat ON pro.cat_id = cat.cat_id  ";
    $output .= "<h4 class='text-center'>Stock Report of ".$date ." </h4>";
}else{
    $sql = "SELECT prod_id,prod_modal,cat_name,prod_qty FROM tbl_products pro JOIN tbl_category cat ON pro.cat_id = cat.cat_id WHERE cat.cat_id='$category'";
   $output .= "<h4 class='text-center'> Stock Report of ".$date ." </h4>";
}
